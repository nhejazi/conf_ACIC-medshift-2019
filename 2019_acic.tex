...
\item A targeted minimum loss estimator (TMLE) may be constructed by using the
  ...
  \textit{targeted} fluctuation processes, respectively,
  $\text{logit}(g_{\delta,n,k\xi}) =
  \text{logit}(g_{\delta,n,(k-1)\xi}) + \xi_{\Delta}^{\text{lfm}}
  \mathbf{\textcolor{red}{H^A_{(k-1)\xi}}}$ and
  $\text{logit}(m_{n,k\xi}) = \text{logit}(m_{n,(k-1)\xi}) +
  \xi_{\Delta}^{\text{lfm}}
...
\item We show the causal parameter $\mathbb{E}\{Y(g_\delta, q)\}$ is identified
  by a functional of the distribution of $O$ \cite{diaz2019causal}:
  \begin{equation*}
