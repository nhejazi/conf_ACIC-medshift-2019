...
\item Using stochastic interventions, we present a decomposition of the
  ...
  \begin{itemize}
    \item Define causal contrasts of effects of continuous and categorical
      exposures.
    \item Identify a parameter required to construct both direct and indirect
      effects.
  \end{itemize}
\item We propose estimators for constructing these direct and indirect effects:
  \begin{itemize}
    \item \textit{Classical}: substitution and IPW estimators using parametric
      methods.
    \item \textit{Nonparamtric-efficient}: one-step and TMLE leveraging machine
      learning.
  \end{itemize}
...
\item A targeted minimum loss estimator (TMLE) may be constructed by using the
  ...
  \begin{equation*}
    \hat{\theta}_{\text{TMLE}}(\delta) = \int m^{\star}_n(z, a, w)
    g_{\delta,n}^{\star}(a \mid w) d\kappa(a),
  \end{equation*}
  where
  $g_{\delta,n}^{\star}(a \mid w)$ and $m^{\star}_n(z,a,w)$ come from
  \textit{targeting} processes using parametric models
  $\text{logit}(g_{\delta,n,k\epsilon}) =
  g_{\delta,n,(k-1)\epsilon} + \epsilon_{\Delta}
  \mathbf{\textcolor{red}{H^A_{(k-1)\epsilon}}}$ and
  $\text{logit}(m_{n,k\epsilon}) = m_{n,(k-1)\epsilon} + \epsilon_{\Delta}
  \mathbf{\textcolor{blue}{H^Y_{(k-1)\epsilon}}}$, respectively.
  \begin{itemize}
    \item Unlike the one-step estimator, TMLE constructs a substitution
      estimator, respecting bounds.
    \item Targeting step uses the method of unverisal least favorable
      one-dimensional submodels \citep{vdl2016one}.
  \end{itemize}
\item Both are multiply robust and allow confidence intervals and hypothesis
  tests to be constructed from an EIF-based variance estimator
  $\mathbb{V}(\hat{\theta}(\delta)) = D^2_{\hat{\eta}, \delta}(O)$,
  valid even when using machine learning.
\end{itemize}
}
