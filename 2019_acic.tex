...
\item A targeted minimum loss estimator (TMLE) may be constructed by using the
  ...
  \end{equation*}
  where
  $g_{\delta,n}^{\star}(a \mid w)$ and $m^{\star}_n(z,a,w)$ are generated via
  parametric \textit{targeting} processes, respectively,
  $\text{logit}(g_{\delta,n,k\epsilon}) =
  \text{logit}(g_{\delta,n,(k-1)\epsilon}) + \epsilon_{\Delta}^{\text{lfm}}
  \mathbf{\textcolor{red}{H^A_{(k-1)\epsilon}}}$ and
  $\text{logit}(m_{n,k\epsilon}) = \text{logit}(m_{n,(k-1)\epsilon}) +
  \epsilon_{\Delta}^{\text{lfm}}
  \mathbf{\textcolor{blue}{H^Y_{(k-1)\epsilon}}}$.
  \begin{itemize}
