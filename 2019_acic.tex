...
\item We propose estimators of these direct and indirect effects:
  \begin{itemize}
    \item \textit{Classical parametric}: substitution and re-weighted (IPW)
      estimators
    \item \textit{Nonparamtric-efficient}: one-step and TML estimators with
      machine learning
  \end{itemize}
\item Our efficient estimators are asymptotically linear under a condition
  requiring $n^{1/4}$-consistency of nuisance regression functions
\end{itemize}
...
\item To avoid entropy conditions on initial estimators, we rely on
  cross-fitting \citep{zheng2011cross, chernozhukov2016double}, letting
...
\item A one-step estimator \citep{pfanzagl1982contributions} may be
  ...
  ($D^A$ and $D^Y$) in the efficient influence function, yielding
  \begin{equation*}
...
\item A targeted minimum loss-based estimator may be constructed by using the
  efficient influence function as an estimating equation, updating components of
  th substitution estimator:
  \begin{equation*}
...
\item We show the causal parameter $\mathbb{E}\{Y(g_\delta, q)\}$ is identified
  ...
  \end{equation*}
  which corresponds to implementing a stochastic intervention (indexed by
  $\delta$) on $A$ while drawing $Z$ as though $A$ were generated by its natural
...
\item Letting $\eta = (g, e, m, \phi)$, the efficient influence function for
  ...
  \begin{align*}
    ...
    D^Y_{\eta, \delta}(o) = \frac{g_{\delta}(a \mid w)}{e(a \mid z, w)}
      \{y - m(z,a,w) \},\\
    D^A_{\eta,\delta}(o) &= \frac{\delta\phi(w)\{a - g(1\mid w)\}}{\{\delta
      g(1\mid w) + 1 - g(1\mid w)\}^2},
    \quad
