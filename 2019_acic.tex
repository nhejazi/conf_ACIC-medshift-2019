...
\item A targeted minimum loss estimator (TMLE) may be constructed by using the
  ...
  \begin{equation*}
    \hat{\theta}_{\text{TMLE}}(\delta) = \int \sum_{i=1}^n
    \hat{m}^{\star}(Z_{j(i)}, a, W_{j(i)})
    \hat{g}_{\delta}^{\star}(a \mid W_{j(i)}) d\kappa(a),
  \end{equation*}
  where
  $\hat{g}_{\delta}^{\star}(a \mid w)$ and $\hat{m}^{\star}(z,a,w)$ are
  generated via \textit{targeted} fluctuation processes, respectively,
